...
std::vector<UnitAttack> AssetDatabase::getAllUnitAttacksByID(const std::vector<int>& attackPrefabIDs) {
    ...
    for(auto& id : attackPrefabIDs) {
        ...
        if(unitAttack.has_value()) {
            ...
            std::cerr << "ERROR: Failed to load attack prefab ID number " << id << std::endl;
        }
    }
    return result;
}
...
Enemy AssetDatabase::getRandomEnemy() {
    if(_enemyPrefabs.empty()) {
        std::cerr << "ERROR: No enemy prefabs found." << std::endl;
...
std::optional<std::string> AssetDatabase::getNameForEnemyType(const std::string &enemyType) {
    auto searchResult = _enemyTypeToNameOptionsMap.equal_range(enemyType);
...
void AssetDatabase::insertDatabase(const std::string& dataLine) {
    ...
    if(lineStream.fail()) {
        ...
        std::cerr << "ERROR: Unknown dataType: " << dataType << std::endl;
    }
}
...
bool AssetDatabase::dumpDatabaseToFile(const std::string& fileName) {
    ...
    for (auto& enemyNamePair : _enemyTypeToNameOptionsMap) {
        outputFile << "ENEMYNAME " << enemyNamePair.first << " " << enemyNamePair.second << std::endl;
    }
